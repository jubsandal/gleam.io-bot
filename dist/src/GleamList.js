...
export async function getGiveawaysWithMethod(browser, searchedMethod) {
    const page = (await browser.pages())[1];
    await page.goto('http://gleamlist.com/?page=1');
    await sleep(2000);
    const rawhrefs = await page.evaluate(() => Array.from(document.querySelectorAll('a[href]'), a => a.getAttribute('href')));
    let hrefs = new Array();
...
export async function getGiveaways(browser, amount, pageNumber) {
    const page = (await browser.pages())[1];
